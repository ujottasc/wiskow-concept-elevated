...
function ColecoesAdmin() {
  ...
  return (
    <div>
          ...
          <div key={c.id} className="bg-card border border-border overflow-hidden">
            ...
            <div className="p-5">
              <div className="flex items-center gap-2 min-w-0">
                <h3 className="font-serif text-xl truncate">{c.name}</h3>
                {c.featured && <Star className="h-3.5 w-3.5 shrink-0 fill-current text-petal" />}
              </div>
              <p className="mt-2 text-xs text-muted-foreground line-clamp-2">{c.description}</p>
              <div className="mt-4 flex gap-2">
                <button onClick={() => { setIsNew(false); setEditing(c); }} className="flex-1 border border-border min-h-11 text-[10px] uppercase tracking-[0.22em] inline-flex items-center justify-center gap-2"><Pencil className="h-4 w-4" /> Editar</button>
                <button onClick={() => { if (confirm(`Remover ${c.name}?`)) void removeCollection(c.id); }} aria-label="Remover" className="border border-border w-11 min-h-11 flex items-center justify-center"><Trash2 className="h-4 w-4" /></button>
              </div>
            </div>
          </div>
        ))}
      ...
      {editing && (
        <AdminDrawer title={isNew ? "Nova coleção" : "Editar coleção"} onClose={() => setEditing(null)}>
