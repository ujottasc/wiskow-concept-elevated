...
function InstagramAdmin() {
  ...
  return (
    <div>
      ...
      <div className="bg-card border border-border p-4 sm:p-6 max-w-4xl space-y-6">
        <div className="flex items-center gap-3">
          <IG className="h-5 w-5 shrink-0" />
          <div className="flex-1 min-w-0">
            ...
            <input value={settings.instagram} onChange={e => updateSettings({ instagram: e.target.value.replace("@", "") })}
              className="w-full sm:max-w-xs border border-border px-3 py-2 text-sm bg-transparent" />
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          <button onClick={async () => { setSaving(true); await saveSettingsNow(); setSaving(false); }}
            className="w-full sm:w-auto bg-foreground text-background px-6 py-3 text-xs uppercase tracking-[0.22em] disabled:opacity-50" disabled={saving}>
            {saving ? "Salvando..." : "Salvar feed"}
          </button>
          {feed.length === 0 && (
            <button onClick={() => updateSettings({ instagramFeed: instagramFeed.slice(0, 12) })}
              className="w-full sm:w-auto border border-border px-6 py-3 text-xs uppercase tracking-[0.22em]">
